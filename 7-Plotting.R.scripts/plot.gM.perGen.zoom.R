###########################################################
## plot.gM.perGen.zoom.R: Script similar to plot.perGen.R but to use with the "File/ Custom plot" option 
## Pauline GARNIER-GERE 
## R script in the gMetapop *.exe folder 
## allows choosing the first and last reproductive events to visualize 
## CHANGES: See plot.gM.perGen.R script for description 
...
## 1) it can be launched with the "File/ Custom plot" option in GUI menu.
## 2) OR using a command line under dos or linux

## 1) The "File/Custom plot" option can be used with any R script having a minimum of 2 arguments:
#      (see part 3.9.2 and various examples in Chapter 5 of the User Manual)
#    a) copy the script in your working folder 
#    b) click "File/Custom plot" from the GUI menu
#    c) in the command window, type: 
#       plot.gM.perGen.zoom.R res1_per_gen_1.txt newplotname 300 1000 
#  argument 1 should be the res1_per_gen_1.txt result file (or any other file with the same structure),
#  argument 2 is the filename of the plot, that will be loaded in the GUI (*.png/*.pdf extensions)
#  argument 3 is first reproductive event for the zooming period  
#  argument 4 is last reproductive event for the zooming period  (if arg 3 and 4 are not provided, the default plot is reproduced)

# this script can be modified to detect additional arguments 
#(for example for selecting which statistics to plot), see the # define argument X ... lines 

## 2) Command line under a Dos or Linux prompt:
# for example
# "C:\path.of.Rscript.exe\Rscript.exe plot.gM.perGen.zoom.R res1_per_gen_1.txt plot.name 500"
...
#            stat2rem=c("Generation","St","Nt","Nremt","Nsext","Nclot"),max.y=1,min.y=-1,plot.legend=T,leg.mar=9,
#            start.re.on.plot=staop,stop.re.on.plot=stoop,plot.dev="png")
             ...
             # leg.mar=9        : allows a size width for the legend rectangle on the right of the plot, can be increased for particular plots
             ...
             # start.re.on.plot : first reproductive event at which statistics are plotted
             # stop.re.on.plot  : last      "         "    "    "      "        "   "
...
options(scipen = 999)
#########################################################################################################"
## A) Fun to extract plot legend info from conf.txt file
...
if (mating.sys=="s"){
fem.fec.mean<-round(mean( as.numeric(strsplit( strsplit(conf[9],split=":")[[1]][2] , split=" ")[[1]])[-1] ),1) 
fem.fec.range<-range( as.numeric(strsplit( strsplit(conf[9],split=":")[[1]][2] , split=" ")[[1]])[-1] )
if (fem.fec.range[1]==fem.fec.range[2]) ffr<-round(fem.fec.range[1],1) else ffr<-paste(round(fem.fec.range[1],1),"-",round(fem.fec.range[2],1),sep="")
ffec.var<-round(var( as.numeric(strsplit( strsplit(conf[9],split=":")[[1]][2] , split=" ")[[1]])[-1] ), 3)
...
## Extract migration rate info
  if (nb.pop>1) {
     ...
     mp.range<-formatC(range(pop.pol2-diag( NA ,nrow=nb.pop,ncol=nb.pop ),na.rm=T) , format="e", digits=2)
...
stop_quietly_zygm <- function() {
  ...
  if (length(grep("\t",pop.zyg[1]))!=0) {
    ...
    pop.zyg2<-apply(as.matrix(pop.zyg,col=length(pop.zyg)/2),1,function(x) as.numeric(strsplit(x,split=" ")[[1]]))                                     
     ...
     mz.range<-formatC(range(pop.zyg2-diag( NA ,nrow=nb.pop,ncol=nb.pop ),na.rm=T) , format="e", digits=2)
...
## extract mutation rates range 
    ...
    if (length(mut.values)==0 | !F %in% is.na(mut.values)) mur<-NA else {
       mut.range<-range(mut.values[which(is.na(mut.values)==F)]) # covers case only cyto loci
       if (mut.range[1]==mut.range[2]) mur<-formatC(mut.range[1], format="e", digits=2) else {
           mur<-paste(formatC(mut.range[1], format="e", digits=2)," ",formatC(mut.range[2],format="e", digits=2),sep="")
       }
       }
    if (length(cymut.values)==0 | !F %in% is.na(cymut.values)) cymur<-NA else {
       ...
       if (cymut.range[1]==cymut.range[2]) cymur<-formatC(cymut.range[1], format="e", digits=2) else {
           cymur<-paste(formatC(cymut.range[1], format="e", digits=2)," ",formatC(cymut.range[2],format="e", digits=2),sep="")
       }       
       }
...
## extract recombination rates range and paternal inheritance value
    ...
    if (length(rec.values)==0 | !F %in% is.na(rec.values)) recr<-NA else {
      rec.range<-range(rec.values[which(is.na(rec.values)==F)]) # covers case of only cyto loci
    if (rec.range[1]==rec.range[2]) recr<-formatC(rec.range[1], format="e", digits=2) else {
        recr<-paste(formatC(rec.range[1], format="e", digits=2)," ",formatC(rec.range[2],format="e", digits=2),sep="")
    }
    }
...
max.y=1,min.y=-1, plot.legend=T,leg.mar=9,start.re.on.plot=NA,stop.re.on.plot=NA,plot.dev=c("png","pdf")  ) {
...
# Computing mean for h2.pan h2.c and fitness if statistics are requested in per_pop output file
  warn.pop.nb<-NULL
  if (length( grep("^res1_per_pop_1.txt$", list.files(, all.files=F,full.names=F, recursive=F) ) )!=0) { # search only for exact string res1_per_pop_1.txt
...
# this should occur only for all loci at once or for none of them
  ...
  stat2drop<-which(datoc==0)  
   if (length( stat2drop)!=0) stat2plot2<-stat2plot[!stat2plot %in% names(stat2drop)] else stat2plot2<-stat2plot               

############################################ filtering perGen for zooming on less reproductive events
  if (is.na(start.re.on.plot)==T) startre<-min(perGen$Gen) else  startre<-as.numeric(start.re.on.plot) 
  if (is.na(stop.re.on.plot)==T) stopre<-max(perGen$Gen) else stopre<-as.numeric(stop.re.on.plot)   
  perGen2<-perGen[which.min(abs(perGen$Gen-startre)):which.min(abs(perGen$Gen-stopre)),]                
  perGen<-perGen2
############################################################################################################
...
if (plot.dev=="pdf") pdf(file=paste(plot.name,".pdf",sep=""), width = 9, height = 9 )
  ...
  bound.df<-sapply(perGen[,stat2plot2,drop=F],minmax)  
  ...
  ylim=c(ylim.minbound,ylim.maxbound), type="l")      #  xlim=c(0,nbgen),
...
############## ERROR messages below would be printed in LOG window
   ## extract nb nuclear loci from stat2plot to cross check nb of nuclear loci from conf.txt
...
"black","black","blue","blue","forestgreen","forestgreen","orange","orange","brown","brown") # maybe use red also for sel since not at same time? until now to update alternative 2 other colour than black for second one
...
list.quanti1<-c("Qst.p","Qst.gc","mh2.p","mh2.gc","mW")
   ...
   for (ssq in list.quanti1) {
      if ( ssq %in% stat2plot2) {
        ...
        # alternative 2
        #lines(perGen$Gen,perGen[,stat2plot2,drop=F][,issq], col = col.quanti[j], type = "p", cex=0.1 )
...
## C) plot legend
  ...
  ## Add info to the legend list
   ...
   add.legend<-c(paste("Ncyto ",pli$nb.cl,sep=""),paste("Ncod ",pli$nb.nl-pli$nb.sel," Nqtl ",pli$nb.sel,sep=""),
                 ...
                 paste("w2.rg ",pli$w2r,sep=""),paste("Vzopt ",round(pli$Vzopt,4),sep=""),
                 paste("nuMut ",pli$mur, sep=""),  # both types of nuc loci
                 paste("Rec ",pli$recr,sep=""), 
                 paste("Al.rg cod/qtl ",pli$nar,"/",pli$sar,sep=""),
                 paste("K.rg ",pli$kccr,sep=""), paste("K.m ",pli$Kcc.mean,sep=""),
                 paste("ffec.m/var ",round(pli$fem.fec.mean,3),"/",round(pli$ffec.var,3),sep=""),paste("ffec.rg ",pli$ffr,sep="") )
          } else {
   add.legend<-c(paste("Ncyto ",pli$nb.cl,sep=""),paste("cyMut ",pli$cymur,sep=""),paste("cyAl.rg ",pli$cyar,sep=""),
   ...
   } else if (extract.char(grep('Kind of sel', conf.2, value=TRUE))=="g") { # selection on genotypes
          if (pli$nb.cl==0) {
   add.legend<-c(paste("Ncyto ",pli$nb.cl,sep=""),
                 ...
                 paste("Rec ",pli$recr,sep=""), 
                 paste("Al.rg cod/sel ",pli$nar,"/",pli$sar,sep=""),
                 paste("K.rg ",pli$kccr,sep=""), paste("K.m ",pli$Kcc.mean,sep=""),
                 paste("ffec.m/var ",round(pli$fem.fec.mean,3),"/",round(pli$ffec.var,3),sep=""),paste("ffec.rg ",pli$ffr,sep="")  )
          } else {
   add.legend<-c(paste("Ncyto ",pli$nb.cl,sep=""),paste("cyMut ",pli$cymur,sep=""),paste("cyAl.rg ",pli$cyar,sep=""),
          ...
          if (pli$nb.cl==0) {
   add.legend<-c(paste("Ncyto ",pli$nb.cl,sep=""),paste("Ncod ",pli$nb.nl,sep=""),paste("nuMut ",pli$mur, sep=""),
                 paste("Rec ",pli$recr,sep=""), 
                 paste("Al.rg cod ",pli$nar,sep=""), paste("K.rg ",pli$kccr,sep=""), paste("K.m ",pli$Kcc.mean,sep=""),
                 paste("ffec.m/var ",round(pli$fem.fec.mean,3),"/",round(pli$ffec.var,3),sep=""),paste("ffec.rg ",pli$ffr,sep="")  )

          } else if (pli$nb.cl!=0 &  pli$nb.nl==0) {
   add.legend<-c(paste("Ncyto ",pli$nb.cl,sep=""),paste("cyMut ",pli$cymur,sep=""),paste("cyAl.rg ",pli$cyar,sep=""),
                 paste("cy.pat.inherit ",pli$cypir,sep=""),
                 paste("Ncod ",pli$nb.nl,sep=""),paste("K.rg ",pli$kccr,sep=""), paste("K.m ",pli$Kcc.mean,sep=""),
                 paste("ffec.m/var ",round(pli$fem.fec.mean,3),"/",round(pli$ffec.var,3),sep=""),paste("ffec.rg ",pli$ffr,sep="")  )
          } else { # both !=0
   add.legend<-c(paste("Ncyto ",pli$nb.cl,sep=""),paste("cyMut ",pli$cymur,sep=""),paste("cyAl.rg ",pli$cyar,sep=""),
                 paste("cy.pat.inherit ",pli$cypir,sep=""),
  ...
  lty = c(true.lty,cytt.lty,true.lty.m,true.ltyq1,add.lty,NA,NA,NA,NA,NA,NA,NA), lwd=2, cex = 0.7 , xpd=TRUE)  #cex for text size of legend vec
  }
...
plot.legend=T,leg.mar=9,start.re.on.plot=NA,stop.re.on.plot=NA) {
...
# if (length(list.files(, all.files=F,full.names=F, recursive=F, pattern="res1_per_pop_1.txt") )!=0) { # Pb if older res1_per_pop_1.txt with added prefix or suffix & exact name not in folder --> cond is T & exact filename not found
  if (length( grep("^res1_per_pop_1.txt$", list.files(, all.files=F,full.names=F, recursive=F) ) )!=0) { # search only for exact string res1_per_pop_1.txt
...
#colors()
 ...
 true.ltyq2<-NULL
  ...
  if (pli$nb.pop==1) {
   ...
   add.legend<-c(paste("cyMut ",pli$cymur,sep=""),paste("cyAl.rg ",pli$cyar,sep=""),paste("cy.pat.inherit ",pli$cypir,sep=""),
                 paste("Ncod ",pli$nb.nl-pli$nb.sel," Nqtl ",pli$nb.sel,sep=""),
                 paste(pli$shsel," selection",sep=""),
                 paste("w2.rg ",pli$w2r,sep=""),paste("Vzopt ",round(pli$Vzopt,4),sep=""),
                 paste("nuMut ",pli$mur, sep=""),
...
result.file<-cmd_args[1] # define argument 1 as a result file with the same structure as the "res1_per_gen_1.txt" file name
...
staop<-cmd_args[3] # 3th argument, start of zooming window for repro events
stoop<-cmd_args[4] # 4th argument, stop of zooming window for repro events

plot.perGen(res.2.plot=result.file,plot.name=plot.na,conf2="conf.txt",def2plot=T,list2plot.user=NULL,
stat2rem=c("Generation","St","Nt","Nremt","Nsext","Nclot"),max.y=1,min.y=-1,plot.legend=T,leg.mar=9,start.re.on.plot=staop,stop.re.on.plot=stoop,plot.dev="png")
plot.perGen(res.2.plot=result.file,plot.name=plot.na,conf2="conf.txt",def2plot=T,list2plot.user=NULL,
stat2rem=c("Generation","St","Nt","Nremt","Nsext","Nclot"),max.y=1,min.y=-1,plot.legend=T,leg.mar=9,start.re.on.plot=staop,stop.re.on.plot=stoop,plot.dev="pdf")
   ## run plot quanti stats list 2
plot.quanti2(res.2.plot.q=result.file,conf3="conf.txt",def2plot=T,list2plot.user=NULL,
stat2rem=c("Generation","St","Nt","Nremt","Nsext","Nclot"),plot.legend=T,leg.mar=9,start.re.on.plot=staop,stop.re.on.plot=stoop)
